...
class Helper:
    ...
    @classmethod
    def notify_server_deply_plan(cls, req):
        ...
        env = AttrDict(
            ...
            SPUG_DEPLOY_PLAN=req.plan
        )
        command = f'cd {extend.dst_dir} && {extend.hook_plan}'
                ...
                with host.get_ssh() as ssh:
                    for code, out in ssh.exec_command_with_stream(command, environment=env):
                        if code != 0:
                            pass
    @classmethod
    def notify_server_deply_plan_cancel(cls, req):
        deploy = req.deploy
        extend = deploy.extend_obj
        env = AttrDict(
            SPUG_APP_NAME=deploy.app.name,
            SPUG_APP_KEY=deploy.app.key,
            SPUG_APP_ID=str(deploy.app_id),
            SPUG_DEPLOY_ID=str(deploy.id),
            SPUG_ENV_ID=str(deploy.env_id),
            SPUG_ENV_KEY=deploy.env.key
        )
        command = f'cd {extend.dst_dir} && {extend.hook_plan_cancel}'
        for h_id in json.loads(deploy.host_ids):
            host = Host.objects.filter(pk=h_id).first()
            if host:
                with host.get_ssh() as ssh:
                    for code, out in ssh.exec_command_with_stream(command, environment=env):
