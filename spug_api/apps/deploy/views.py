    ...
    def delete(self, request):
        ...
        if error is None:
            if form.id:
                ...
                deploy.delete()
                Thread(target=Helper.notify_server_deply_plan_cancel, args=(deploy,)).start()
...
class RequestDetailView(View):
    ...
    def patch(self, request, r_id):
        ...
        if error is None:
            ...
            req.save()
            Thread(target=Helper.send_deploy_notify, args=(req)).start()
        return json_response(error=error)
...
def post_request_ext1(request):
    ...
    if error is None:
        ...
        logging.warning(f'!!! post_request_ext1 extend= {extend.hook_plan}')
        if extend.hook_plan and req.plan != None:
            Thread(target=Helper.notify_server_deply_plan, args=(req,)).start()
    return json_response(error=error)
