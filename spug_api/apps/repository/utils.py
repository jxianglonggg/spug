...
def _build(rep: Repository, helper, env):
    ...
    if extras[0] == 'branch':
        SPUG_GIT_BRANCH=extras[1]
        SPUG_GIT_COMMIT_ID = extras[2]
        command = f'cd {git_dir} && git switch {SPUG_GIT_BRANCH} && git checkout {SPUG_GIT_COMMIT_ID} && git submodule update --init --recursive && git-archive-all.sh -t {tree_ish} --prefix {rep.spug_version}/  --format tar && mv {str(rep.deploy_id)}.tar ../'
        helper.send_step('local', 2, f'{human_time()} 执行检出...        ')
        helper.local(command)
    else:
        SPUG_GIT_TAG=extras[1]
        command = f'cd {git_dir} && git checkout -b {SPUG_GIT_TAG} && git submodule update --init --recursive && git-archive-all.sh -t {tree_ish} --prefix {rep.spug_version}/  --format tar && mv {str(rep.deploy_id)}.tar ../'
        helper.send_step('local', 2, f'{human_time()} 执行检出...{command}\n')
        helper.local(command)
    command = f'cd {REPOS_DIR} && ls && tar xf {str(rep.deploy_id)}.tar && rm {str(rep.deploy_id)}.tar'
    helper.local(command)
    helper.send_info('local', '\033[32m完成√\033[0m\r\n')
